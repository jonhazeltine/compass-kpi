...
| Coach Content Library (`coach_content_library`) | Coach, Admin operator, Challenge Sponsor (limited sponsor-scoped access/linking) | `coaching_content`, `communication` templates | `coaching_journeys*`, `inbox*` template usage | `⚪ planned` | Authoring/curation only; runtime delivery never edits canonical content here. Library uses folder/files IA (`Collections -> Assets`) with upload intake included and no duplicate Assets tab model. Sponsor access is scope-limited to sponsor campaign contexts and does not grant KPI logging actions. |
| Journey Authoring Studio (`coach_journey_authoring`) | Coach | `coaching_content` | `coaching_journeys*` published journey versions | `⚪ planned` | Draft/review/publish lifecycle is ops concern, not member runtime concern. Journey Builder uses two-column authoring (`source pane` + `journey builder`) with `Create New Journey` and mandatory drag/drop runtime behaviors (`asset<->collection`, `asset->journey`, `reorder/remove`) plus in-builder `Save Draft` status states (`idle`, `pending`, `saved`, `error`). |
...
| Channel Operations / Comms Hub (`channels`) | Coach, Admin operator, Challenge Sponsor (limited sponsor scopes) | `communication` | complements runtime `inbox_channels` / `channel_thread` channel context ops | `🟡 partial` | Portal `channels` surface now mirrors comms-hub IA tabs (`All`, `Channels`, `DMs`, `Broadcast`) with scope filters (`Team`, `Cohort`, `Segment`, `Global`) and role-gated broadcast visibility. Sponsor access remains comms-only and sponsor-scoped; deeper moderation/template lifecycle controls are follow-up. |
...
- `/coach/channels` is the primary communications hub entry for coach-facing channel operations and comms context management.
