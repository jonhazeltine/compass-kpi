...
### W12 Comms Hub IA Model (Implemented Runtime Direction)

- Tab model: `All`, `Channels`, `DMs`, `Broadcast`.
- Channel scope filters: `Team`, `Cohort`, `Segment`, `Global` (server-scoped visibility).
- Broadcast visibility:
  - visible to Coach/Admin/Team Leader where role+scope allow.
  - hidden for Team Member/Solo by default.
  - Sponsor stays sponsor-scoped comms participant; no KPI logging actions.
- Runtime route mapping:
  - `All` -> `inbox`
  - `Channels` -> `inbox_channels`
  - `DMs` -> `channel_thread` (direct-thread context)
  - `Broadcast` -> `coach_broadcast_compose` (role-gated)
- Coach portal mapping:
  - `/coach/channels` is the comms-hub host surface using the same IA model.
  - `/admin/coaching/channels` remains compatibility alias routing to `/coach/channels`.
...
| Inbox / Channels dedicated flow | Team Leader, Team Member, Solo User | `communication` | `inbox`, `inbox_channels`, `channel_thread` | `🟡 stub` | `🟡 partial` | dedicated flow shell | IA model is `All / Channels / DMs / Broadcast` with server-scoped filters (`Team`, `Cohort`, `Segment`, `Global`); Team Member/Solo do not receive broadcast compose visibility. W4 adds API-backed channel list fetch (`GET /api/channels`), thread reads (`GET /api/channels/{id}/messages`), message send (`POST /api/channels/{id}/messages`), and mark-seen (`POST /api/messages/mark-seen`) with context-aware filtering/fallbacks. Runtime package visibility banners/fallback states are now rendered; blocked/gated action disable depends on explicit server outcomes. |
| Broadcast composer (leader/admin role-gated) | Team Leader (Admin/Coach later per DEP-003) | `communication` | `coach_broadcast_compose` | `🟡 stub` | `🟡 partial` | Team Dashboard + Inbox role-gated CTA | Broadcast tab maps to `coach_broadcast_compose` only for authorized roles (Coach/Admin/Team Leader) and allowed target scopes (`Team`, `Cohort`, `Segment`, `Global`) with server authz enforcement. W4 wires leader broadcast send via `/api/channels/{id}/broadcast` (channel-based path) with UI role gating + API error handling; server remains source of permission/throttle enforcement. Package visibility gating banners are UI-only until explicit entitlement outcomes are provided. |
...
|---|---|---|---|---|---|
| Library Upload Intake (`coach_content_library` capability) | Coach, Admin operator, Team Leader (team-scoped uploads only), Challenge Sponsor (limited sponsor campaign uploads) | `coaching_content`, sponsor campaign assets | `coach_content_library` ingestion -> runtime linked content/templates | `⚪ planned` | Upload intake is subsumed within Library and is not a first-class coach tab. Library IA is `Assets + Collections`; Team Leader uploads are own-team scoped (no org-wide authoring ownership, no sponsor package authority); no KPI logging/edit actions. |
| Coach Content Library (`coach_content_library`) | Coach, Admin operator, Challenge Sponsor (limited sponsor-scoped access/linking) | `coaching_content`, `communication` templates | `coaching_journeys*`, `inbox*` template usage | `⚪ planned` | Canonical top-level mode `Journeys` contains source-pane library tree. Folder/files behavior is `Collections -> nested Assets` with multi-expand and no duplicate Assets tab model. Sponsor access is scope-limited to sponsor campaign contexts and does not grant KPI logging actions. |
| Journey Authoring Studio (`coach_journey_authoring`) | Coach | `coaching_content` | `coaching_journeys*` published journey versions | `⚪ planned` | `Journeys` workspace is split layout (`source pane` + `journey builder`). Supports `Create New Journey` and mandatory drag/drop runtime mapping from assets into lesson/task slots (`asset<->collection`, `asset->lesson/task`, `reorder/remove`) plus in-builder `Save Draft` status states (`idle`, `pending`, `saved`, `error`). |
| People Workspace (`coach_people_workspace`) | Coach, Admin operator, Challenge Sponsor (limited sponsor scopes) | targeting/visibility + communications (`communication`, `sponsor_challenge_coaching`) | runtime cohort-based channels + package targeting inputs | `⚪ planned` | Canonical second top-level mode `People` uses split layout: people list + cohorts/channels panel set. Cohort visibility/constraints only; no challenge participation ownership or KPI logging actions. |
| Channel Operations / Comms Hub (`channels`) | Coach, Admin operator, Challenge Sponsor (limited sponsor scopes) | `communication` | complements runtime `inbox_channels` / `channel_thread` channel context ops | `🟡 partial` | Channel operations are a split panel inside `People` workspace and preserve comms-hub IA tabs (`All`, `Channels`, `DMs`, `Broadcast`) with scope filters (`Team`, `Cohort`, `Segment`, `Global`) and role-gated broadcast visibility. |
...
| `admin/coaching/channels` | `Admin Shell extension` | `channels` | Coach, Admin operator, Challenge Sponsor (limited sponsor scopes) | `🟡 partial` | Compatibility alias resolves to canonical `/coach/channels` comms-hub baseline; advanced portal channel ops remain follow-up work. |
...
- Coach portal primary IA uses exactly two top-level workspace modes: `journeys` and `people`.
- `Journeys` mode = Library tree/source + Journey Builder split workspace.
- `People` mode = People list + Cohorts/Channels split workspace.
- `/coach/channels` remains a compatibility entry into the People workspace channel split context (not a separate top-level mode).
- Drag `Library -> Journey` is the primary authoring interaction target for coach content composition.
- Drag/drop interactions in Journey Builder are mandatory runtime behavior (not placeholder affordances).
...
- `Tabs/Segmented Controls`: section switching only (top-level workspace mode switch `Journeys`/`People` and local section pivots).
