...
| Coach Content Library (`coach_content_library`) | Coach, Admin operator, Challenge Sponsor (limited sponsor-scoped access/linking) | `coaching_content`, `communication` templates | `coaching_journeys*`, `inbox*` template usage | `⚪ planned` | Authoring/curation only; runtime delivery never edits canonical content here. Library is the canonical `Assets + Collections` workspace, including upload intake. Sponsor access is scope-limited to sponsor campaign contexts and does not grant KPI logging actions. |
| Journey Authoring Studio (`coach_journey_authoring`) | Coach | `coaching_content` | `coaching_journeys*` published journey versions | `⚪ planned` | Draft/review/publish lifecycle is ops concern, not member runtime concern. Journey Builder includes `Create New Journey` (blank + naming), mandatory drag/drop interactions for assignment/reorder/remove, and in-builder action-bar `Save Draft` status states (`idle`, `pending`, `saved`, `error`). |
