...
| Channel Operations / Comms Hub (`channels`) | Coach, Admin operator, Challenge Sponsor (limited sponsor scopes) | `communication` | complements runtime `inbox_channels` / `channel_thread` channel context ops | `⚪ planned` | Portal `channels` surface is the primary coach communications hub; sponsor access is comms-only and sponsor-scoped. |
