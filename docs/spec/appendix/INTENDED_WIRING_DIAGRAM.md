...
| Channel Operations / Comms Hub (`channels`) | Coach, Admin operator, Challenge Sponsor (limited sponsor scopes) | `communication` | complements runtime `inbox_channels` / `channel_thread` channel context ops | `🟡 partial` | Portal `channels` surface now has runtime baseline in `CoachPortalScreen` (`/coach/channels` roster + scoped context framing). Sponsor access remains comms-only and sponsor-scoped; deeper moderation/template lifecycle controls are follow-up. |
