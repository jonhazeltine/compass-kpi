...
exports.canAuthorCoachTask = canAuthorCoachTask;
exports.buildLinkedTaskRights = buildLinkedTaskRights;
exports.canMutateLinkedTask = canMutateLinkedTask;
exports.validateChannelMessagePayload = validateChannelMessagePayload;
exports.serializeChannelMessageBody = serializeChannelMessageBody;
exports.parseTaskAssignmentRef = parseTaskAssignmentRef;
exports.buildTaskAssignmentRef = buildTaskAssignmentRef;
exports.buildTaskMessageBody = buildTaskMessageBody;
exports.buildChannelMessageReadModel = buildChannelMessageReadModel;
...
function normalizeTaskStatus(raw) {
    ...
    if (normalized === "completed" || normalized === "done")
        return "completed";
    if (normalized === "in_progress" || normalized === "active" || normalized === "blocked")
        return "in_progress";
    return "pending";
}
function canAuthorCoachTask(role) {
    return role === "coach" || role === "team_leader" || role === "admin" || role === "super_admin";
}
function buildLinkedTaskRights(input) {
...
function canMutateLinkedTask(input) {
...
function normalizeTaskCardDraft(raw) {
    if (!isRecord(raw))
        return undefined;
...
function validateChannelMessagePayload(body) {
    ...
    if (taskAction === "create") {
        if (!taskDraft.title)
            return { ok: false, status: 422, error: "task_card_draft.title is required for task create" };
...
function parseTaskAssignmentRef(raw) {
    if (!isRecord(raw))
        return null;
...
function buildTaskAssignmentRef(input) {
    return {
        id: input.taskId,
...
function buildTaskMessageBody(input) {
    const note = typeof input.note === "string" ? input.note.trim() : "";
    if (note)
        return note;
    if (input.taskAction === "complete")
        return `Completed task: ${input.title}`;
    if (input.taskAction === "update")
        return input.description?.trim() || `Updated task: ${input.title}`;
    return input.description?.trim() || input.title;
}
function buildChannelMessageReadModel(row, viewer) {
    const base = {
        id: String(row.id ?? ""),
    ...
    const lifecycle = lifecycleRaw &&
        ...
        ? {
            processing_status: lifecycleRaw.processing_status,
