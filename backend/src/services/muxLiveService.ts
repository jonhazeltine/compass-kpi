...
export type LiveSessionCreatePayload = {
  channel_id: string;
  title: string;
  starts_at?: string;
  ends_at?: string;
...
export type LiveSessionJoinTokenPayload = {
...
export type LiveSessionRecord = {
  ...
  created_at: string;
  updated_at: string;
  /* Mux provider state (populated when provider_mode === "mux") */
  mux_live_stream_id: string | null;
  mux_stream_key: string | null;
  mux_playback_id: string | null;
  mux_stream_status: string | null; // "idle" | "active" | "disabled"
};

export type LiveSessionPublicView = Omit<LiveSessionRecord, "mux_stream_key">;
...
export function getLiveSession(sessionId: string): LiveSessionRecord | undefined {
  return liveSessionStore.get(sessionId);
}

export function getIdempotentSession(userId: string, idempotencyKey: string): LiveSessionRecord | undefined {
...
export function resolveLiveProviderMode(): LiveProviderMode {
  ...
  if (tokenId && tokenSecret) return "mux";
...
export function canHostLiveSession(role: string): boolean {
...
export function validateLiveSessionCreatePayload(body: unknown):
...
export function validateLiveSessionJoinTokenPayload(body: unknown):
  | { ok: true; payload: LiveSessionJoinTokenPayload }
  | { ok: false; status: number; error: string } {
  if (body === undefined || body === null) return { ok: true, payload: {} };
  if (!isRecord(body)) return { ok: false, status: 400, error: "Body must be a JSON object" };
...
type MuxLiveStreamData = {
  id: string;
  stream_key: string;
  status: string; // "idle" | "active" | "disabled"
...
export async function createMuxLiveStream(): Promise<
  | { ok: true; data: MuxLiveStreamData }
...
> {
  try {
    ...
    const json = (await res.json().catch(() => ({}))) as { data?: MuxLiveStreamData; error?: { messages?: string[] } };
 ...
 */
export async function getMuxLiveStreamStatus(streamId: string): Promise<
  | { ok: true; data: MuxLiveStreamData }
  | { ok: false; error: string }
> {
  try {
    ...
    const json = (await res.json().catch(() => ({}))) as { data?: MuxLiveStreamData };
    if (!res.ok || !json.data) {
 ...
 */
export async function disableMuxLiveStream(streamId: string): Promise<
  | { ok: true }
  | { ok: false; error: string }
> {
  try {
    ...
    if (!res.ok) {
      const json = (await res.json().catch(() => ({}))) as { error?: { messages?: string[] } };
...
/* ================================================================
   MUX ASSET QUERY HELPERS (for replay flow)
   ================================================================ */

type MuxLiveStreamDetail = MuxLiveStreamData & {
 ...
 */
export async function getMuxLiveStreamDetails(streamId: string): Promise<
  | { ok: true; data: MuxLiveStreamDetail }
  | { ok: false; error: string }
> {
  try {
    const res = await fetch(`${MUX_API_BASE}/live-streams/${encodeURIComponent(streamId)}`, {
    ...
    const json = (await res.json().catch(() => ({}))) as { data?: MuxLiveStreamDetail };
    if (!res.ok || !json.data) {
      return { ok: false, error: `Mux API returned ${res.status}` };
    }
    return { ok: true, data: json.data };
...
> {
  try {
    ...
    const json = (await res.json().catch(() => ({}))) as { data?: MuxAssetData };
    if (!res.ok || !json.data) {
      return { ok: false, error: `Mux API returned ${res.status}` };
    }
    return { ok: true, data: json.data };
...
function buildMuxUrls(streamKey: string, playbackId: string | null): {
  host_url: string;
  join_url: string;
  live_url: string;
} {
...
function buildMockUrls(sessionId: string, channelId: string, role: string, token?: string): {
  host_url: string;
  join_url: string;
  live_url: string;
} {
...
export function issueLiveSessionJoinToken(input: {
  sessionId: string;
  channelId: string;
  userId: string;
  role: LiveRole;
...
export async function createSession(input: {
  userId: string;
  payload: LiveSessionCreatePayload;
}): Promise<
  | { ok: true; session: LiveSessionRecord; idempotent_replay: boolean }
  | { ok: false; status: number; error: string }
> {
  ...
  if (existingId) {
    ...
    if (existing) return { ok: true, session: existing, idempotent_replay: true };
  }
  ...
  let muxStreamStatus: string | null = null;
  let urls: { host_url: string; join_url: string; live_url: string };
 ...
 */
export async function refreshSession(sessionId: string): Promise<LiveSessionRecord | undefined> {
...
export function issueJoinToken(input: {
  ...
  const role: LiveRole = session.host_user_id === userId ? "host" : "viewer";
...
export async function endSession(sessionId: string): Promise<
  | { ok: true; session: LiveSessionRecord }
  | { ok: false; error: string }
> {
  const session = liveSessionStore.get(sessionId);
 ...
 */
export function buildSessionResponse(session: LiveSessionRecord, callerIsHost: boolean): {
  session: LiveSessionPublicView;
  provider: string;
  provider_mode: LiveProviderMode;
  host_url: string;
  join_url: string;
