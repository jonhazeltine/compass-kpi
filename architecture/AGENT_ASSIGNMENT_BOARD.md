...
| `COACHING-SAMPLE-CONTENT-SEED-A` | `in_progress` | `M6 coaching slice (runtime realism / validation data)` | `Coach`, `Team Leader`, `Team Member`, `Solo User` | `coaching content delivery` (`journeys`, `lessons`, `assignments`, sample progress/messages`) | coaching journeys/lessons runtime surfaces + comms shells (`coaching_journeys*`, `inbox*`, Team/Challenge coaching modules) | `Mobile-2` | `codex/a2-admin-list-usability-pass` (backend/data worktree strongly preferred) | N/A (data/backend seed swath; no Figma requirement) | New realism follow-up in progress: extend sponsor/cohort fixture depth and add repeatable smoke checks for coach, team-leader, challenge-sponsor, and member visibility outcomes across existing endpoint families. |
...
| `COACHING-W7-COACH-PORTAL-FOUNDATION-SURFACES-A` | `committed` | `W7/W8 coach portal implementation spec pack (docs-only; implementation-ready)` | `Coach`, `Team Leader` (team-scoped content upload only), `Challenge Sponsor` (sponsor-scoped tools), `Admin operator` (limited governance) | `coach portal / content operations` (`surface contracts + role gates + lane checklists`) | coach web portal `content_upload`, `content_library`, `journeys`, `cohorts`, `channels` + implementation-ready admin/mobile checklists (audit remains secondary-only) | `Admin-1` | `codex/a2-admin-list-usability-pass` (coach/admin web worktree preferred) | manual-spec-driven + W6/W7 rescope package | Committed docs swath: finalized W7/W8 implementation spec pack with explicit surface contracts, role gates, sponsor constraints, Team Leader team-scoped upload limits, no-sponsor-KPI-logging enforcement, and implementation-ready checklists for admin + mobile lanes; `/admin/coaching/audit` remains secondary-only governance/troubleshooting. |
...
| `COACHING-W8-RUNTIME-STATE-HARDENING-A` | `active` | `W8 runtime hardening (coach/sponsor/team-leader state resilience)` | `Coach`, `Challenge Sponsor`, `Team Leader`, `Team Member`, `Solo User` | `coaching runtime states` (`loading/empty/error/permission-denied/partial-read-model`) | `KPIDashboardScreen` runtime surfaces: `Home`, `Team`, `Challenge`, `coaching_journeys*`, `inbox*`, `channel_thread` | `Mobile-1` | `codex/a2-admin-list-usability-pass` (dedicated mobile worktree preferred) | manual-spec-driven + accepted W6/W7 runtime stack | Owner-directed W8 swath in progress: harden runtime states/copy/transitions and capture screenshot evidence for key state classes without backend/schema/API changes. |
