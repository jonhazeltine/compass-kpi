...
| `COACHING-W11-COACH-PORTAL-STANDALONE-UX-PASS-A` | `active` | `M6 W11 coach portal standalone UX pass` | `Coach` (primary), `Team Leader` (team-scoped upload), `Challenge Sponsor` (sponsor-scoped tools) | `coach portal IA/UX` (`standalone look`, `de-admin chrome`, `coach-first content workflows`) | canonical `/coach/*` routes with standalone shell treatment and minimized admin framing | `Admin-1` | `codex/a2-admin-list-usability-pass` (coach/admin web worktree preferred) | manual-spec-driven + accepted W10 route baseline | Active hard-correction+polish pass: split `/coach/*` onto a dedicated coach host (outside `AdminShellScreen`), preserve role gates and sponsor no-KPI-logging boundaries, keep `/admin/coaching/*` compatibility redirects, and log structural boundary decision in `DECISIONS_LOG.md`. |
