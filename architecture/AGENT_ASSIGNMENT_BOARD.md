...
| `COACHING-SAMPLE-CONTENT-SEED-A` | `review` | `M6 coaching slice (runtime realism / validation data)` | `Coach`, `Team Leader`, `Team Member`, `Solo User` | `coaching content delivery` (`journeys`, `lessons`, `assignments`, sample progress/messages`) | coaching journeys/lessons runtime surfaces + comms shells (`coaching_journeys*`, `inbox*`, Team/Challenge coaching modules) | `Mobile-2` | `codex/a2-admin-list-usability-pass` (backend/data worktree strongly preferred) | N/A (data/backend seed swath; no Figma requirement) | Review-ready realism expansion: seeded sponsor-linked challenge fixtures + cohort channels + KPI visibility examples and expanded smoke verification across coaching/channels/dashboard/sponsored endpoints in existing families. |
