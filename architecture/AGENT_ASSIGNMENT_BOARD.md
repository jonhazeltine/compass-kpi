...
| `COACHING-W7-COACH-PORTAL-FOUNDATION-SURFACES-A` | `committed` | `W7 coaching portal track (coach web portal surface foundation)` | `Coach`, `Team Leader` (team-scoped content upload only), `Challenge Sponsor` (sponsor-scoped tools), `Admin operator` (limited governance) | `coach portal / content operations` (`IA + surface shells`) | coach web portal `content_upload`, `content_library`, `journeys`, `cohorts`, `channels` (companion to runtime host surfaces/channels, not replacement; sponsor persona gets scoped access) | `Admin-1` | `codex/a2-admin-list-usability-pass` (coach/admin web worktree preferred) | manual-spec-driven + W6/W7 rescope package | Committed implementation phase: admin-shell coach portal foundation route shells landed for `/admin/coaching/uploads`, `/admin/coaching/library`, `/admin/coaching/cohorts`, `/admin/coaching/channels` with role-gated boundaries (Coach primary, Team Leader upload-only team scope, Challenge Sponsor sponsor-scoped), explicit no-KPI-logging action boundaries, and `/admin/coaching/audit` retained as secondary troubleshooting/governance. |
