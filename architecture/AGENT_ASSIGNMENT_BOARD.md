...
| `COACHING-SAMPLE-CONTENT-SEED-A` | `in_progress` | `M6 coaching slice (runtime realism / validation data)` | `Coach`, `Team Leader`, `Team Member`, `Solo User` | `coaching content delivery` (`journeys`, `lessons`, `assignments`, sample progress/messages`) | coaching journeys/lessons runtime surfaces + comms shells (`coaching_journeys*`, `inbox*`, Team/Challenge coaching modules) | `Mobile-2` | `codex/a2-admin-list-usability-pass` (backend/data worktree strongly preferred) | N/A (data/backend seed swath; no Figma requirement) | Follow-up realism swath in progress: expanding seed fixtures and smoke checks for sponsor-scoped content depth, cohort channels, and member KPI visibility examples while keeping existing endpoint families. |
...
| `COACHING-W7-RUNTIME-COACH-SPONSOR-VISIBILITY-A` | `active` | `W7 runtime coaching visibility swath (mobile)` | `Coach`, `Challenge Sponsor`, `Team Leader`, `Team Member`, `Solo User` | `coaching / notifications / participation visibility` (`coach+sponsor runtime modules + cohort-channel context`) | `KPIDashboardScreen` host/runtime surfaces: `Home`, `Team`, `Challenge`, `coaching_journeys*`, `inbox*`, `channel_thread` | `Mobile-1` | `codex/a2-admin-list-usability-pass` (dedicated mobile worktree preferred) | manual-spec-driven + accepted W6/W7 rescope package + existing read-model outputs | Owner-directed runtime swath execution in progress: implement coach+sponsor runtime visibility modules (including cohort channel context and team-leader creator+participant behavior) with no backend/schema/API changes and no sponsor KPI logging actions. |
