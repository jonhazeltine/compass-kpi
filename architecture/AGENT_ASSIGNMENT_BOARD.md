...
| `COACHING-W7-SUPERADMIN-AI-TROUBLESHOOTING-AUDIT-A` | `committed` | `W7 optional hardening (super-admin AI troubleshooting only)` | `Super Admin` (primary), `Admin operator` (limited) | `AI troubleshooting / audit` (`exception-only`) | optional trimmed `/admin/coaching/audit` super-admin troubleshooting views | `Admin-1` | `codex/a2-admin-list-usability-pass` (admin web worktree preferred) | manual-spec-driven + W6/W7 rescope package + explicit owner approval gate | Committed (2026-02-27, `e988d96`): `/admin/coaching/audit` repurposed to exception-only super-admin AI troubleshooting surface, W6 notification-pref/visibility companion blocks removed, super-admin-only queue mutation enforced with platform-admin read-only mode, and validations passed (`npm run test:unit -- adminAuthz`, `npx tsc --noEmit`, runtime route check + screenshots in `app/test-results/coaching-w7-superadmin-ai-troubleshooting-audit-a/`). |
