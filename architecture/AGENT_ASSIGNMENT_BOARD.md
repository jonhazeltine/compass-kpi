...
| `COACHING-W12-COACH-PORTAL-WORKSPACE-IA-E` | `committed+pushed` | `M6 W12 coach portal workspace IA replacement` | `Coach` (primary), `Team Leader` (team-scoped authoring), `Challenge Sponsor` (sponsor-scoped read-only) | `coach portal IA` (`two-mode workspace shell`, `Journeys split view`, `People split view`) | `/coach/library`, `/coach/journeys`, `/coach/cohorts`, `/coach/channels` (+ `/coach/uploads` and `/admin/coaching/*` compatibility checks) | `Admin-1` | `codex/a2-admin-list-usability-pass` (coach/admin web worktree preferred) | owner hard IA replacement (W12 E) | Review-ready extension pass (2026-02-27, Admin-1): `/coach/journeys` now enforces single active-journey editing with row-select context swap, explicit `Add Lesson`/`Add Task` controls, pointer/drag reorder for lessons/tasks, and visible drag lifecycle states for Library -> Journey drop targets (picked/hover/success), while preserving sponsor view-only gate and compatibility redirects. Validation passed (`tsc`, runtime checks + screenshots under `app/test-results/w12-journey-single-editor/`). |
...
| `COACHING-W7-SUPERADMIN-AI-TROUBLESHOOTING-AUDIT-A` | `committed+pushed` | `W7 optional hardening (super-admin AI troubleshooting only)` | `Super Admin` (primary), `Admin operator` (limited) | `AI troubleshooting / audit` (`exception-only`) | optional trimmed `/admin/coaching/audit` super-admin troubleshooting views | `Admin-1` | `codex/a2-admin-list-usability-pass` (admin web worktree preferred) | manual-spec-driven + W6/W7 rescope package + explicit owner approval gate | Committed and pushed (2026-02-27, `e988d96`): `/admin/coaching/audit` repurposed to exception-only super-admin AI troubleshooting surface, W6 notification-pref/visibility companion blocks removed, super-admin-only queue mutation enforced with platform-admin read-only mode, and validations passed (`npm run test:unit -- adminAuthz`, `npx tsc --noEmit`, runtime route check + screenshots in `app/test-results/coaching-w7-superadmin-ai-troubleshooting-audit-a/`). |
| `COACHING-W12-MOBILE-NAV-LOG-CTA-CENTER-A` | `closed` | `M6 / W12 mobile IA polish` | `Coach`, `Team Leader`, `Team Member`, `Solo User`, `Sponsor` | `mobile IA` (`bottom tab LOG center CTA emphasis`) | `KPIDashboardScreen` bottom tab bar only | `Mobile-2` | `codex/a2-admin-list-usability-pass` (mobile app worktree preferred) | N/A (IA/interaction polish; no Figma lock specified in assignment) | Locked baseline landed. Do not relaunch; open new targeted bugfix assignment if regressions are discovered. |
| `M6-W12-TEAM-LEADER-ROW-EXPANSION-AND-LOG-WIRING-A` | `closed` | `M6 / W12` | `Team Leader` (primary), `Team Member` (no-regression) | `team leader ops` (`row expansion UI`, `team KPI -> log context wiring`) | `KPIDashboardScreen` Team Leader surface + handoff to Log interface | `Mobile-2` | `codex/a2-admin-list-usability-pass` (mobile app worktree preferred) | N/A (interaction model + wiring fix) | Locked baseline landed. Do not relaunch; open new targeted bugfix assignment if regressions are discovered. |
...
| `W13-DEP-GATES-CLOSEOUT-TRACKER-A` | `committed` | `W13 docs/control-plane exception` | `Controller`, `Architecture`, `Security/Legal`, `backend/platform` | `dependency closure tracking` (`DEP-002/004/005 go/no-go tracker`) | docs-only: project control plane + assignment board closeout table | `Coach-1` | `codex/a2-admin-list-usability-pass` (docs-only) | N/A (docs governance) | Committed closeout update (owner directive): `DEP-002`/`DEP-004`/`DEP-005` set to `CLOSED` with owner risk-acceptance note and Wave A matrix updated to `GO` with consistent dependency/evidence/sign-off statuses. |
...
| `M6-COMMS-DIRECT-ENTRY-CHANNELS-A` | `review` | `M6 comms runtime IA simplification` | `Team Leader`, `Team Member`, `Coach`, `Solo User`, `Challenge Sponsor` | `comms entry routing` (`direct channels entry`, `inbox alias compatibility`) | `CommsHub`, `KPIDashboardScreen` comms surfaces (`inbox`, `inbox_channels`) | `Mobile-1` | `codex/a2-admin-list-usability-pass` | assignment-directed (no role gate/token-sync drift) | Completed: comms default entry now resolves to `inbox_channels`; standalone Comms Inbox landing/`Open Channels` gateway screen removed from CommsHub render path; legacy `inbox` route retained as compatibility alias mapped to `inbox_channels`; role-gating and Stream token/sync flows unchanged. Validation: `cd app && npx tsc --noEmit --pretty false` pass. Evidence folder: `app/test-results/m6-comms-direct-entry-channels-a/` (member direct-channel first-open screenshot captured; team_leader first-open capture blocked by local macOS assistive-access restriction for simulator UI automation). |
| `M6-CHALLENGE-COACH-TAB-DECOUPLE-A` | `review` | `M6 runtime navigation decoupling` | `Team Leader`, `Team Member`, `Coach`, `Solo User`, `Challenge Sponsor` | `challenge/coach routing` (`separate runtime tabs`, `team top-tab challenge continuity`) | `KPIDashboardScreen` (`bottom tab routing`, `coach subflow`, `challenge flow`) | `Mobile-1` | `codex/a2-admin-list-usability-pass` | assignment-directed (routing/state only; no UI redesign) | Completed: introduced distinct `coach` tab state and decoupled coach/challenge routing. Bottom Coach icon now routes to coach-only surface; Team top-tab `Challenges` routes directly to challenge list/detail/leaderboard surface. Existing coach subflow (`coach_marketplace`/`coach_hub_primary`/etc.) and challenge flow state machine (`list`/`details`/`leaderboard`) preserved. Validation: `cd app && npx tsc --noEmit --pretty false` pass. Evidence: `app/test-results/m6-challenge-coach-tab-decouple-a/team-top-tab-challenges-content.png`; coach-tab screenshot capture is currently blocked by local simulator input automation restriction (assistive-access unavailable), while runtime state capture is at `app/test-results/m6-challenge-coach-tab-decouple-a/runtime-current-screen.png`. |
| `M6-TEAM-PROFILE-DM-HANDOFF-FIX-A` | `review` | `M6 runtime messaging handoff hardening` | `Team Leader`, `Team Member` | `team profile -> comms` (`message CTA handoff`, `direct-channel-first routing`, `DM fallback`) | `KPIDashboardScreen` Team profile drawer + Comms handoff path (optionally `CommsHub` search focus support) | `Mobile-1` | `codex/a2-admin-list-usability-pass` | assignment-directed (routing/state only; no backend/schema changes) | Completed: Team profile drawer `💬 Message` now always routes to Messages with DMs-first state (`setActiveTab('comms')` via shell routing + `setCommsHubPrimaryTab('dms')`). Handoff resolves existing direct channels first (match by `direct/dm` type, then `context_id === member.id`, then normalized name match) and opens `channel_thread`; if none exists, it opens `inbox_channels` DMs list with search prefilled to member name and no dead thread target. Team profile drawer now closes on handoff. Added DMs search autofocus when prefilled for faster start. Validation: `cd app && npx tsc --noEmit --pretty false` pass. Evidence folder: `app/test-results/m6-team-profile-dm-handoff-fix-a/` (`dm-fallback-list-state.png`, `direct-channel-thread-state.png`, `runtime-current-screen.png`); live tap-sequence capture from Team profile remains limited by local simulator input automation restrictions (assistive-access unavailable). |
...
| `M6-DM-REAL-TEAM-MEMBERS-UNBLOCK-A` | `review` | `M6 comms runtime identity hardening` | `Team Leader`, `Team Member` | `team profile/search -> direct DM` (`real roster ids`, `direct thread create-or-resolve`) | `backend /teams/:id`, `KPIDashboardScreen` team/comms DM paths | `Mobile-1` | `codex/a2-admin-list-usability-pass` | assignment-directed (existing endpoint families only) | Completed: `GET /teams/{id}` now returns roster-safe member summaries (`user_id`, `role`, `full_name`, `email`, `avatar_url`), app Team/Comms DM paths now consume roster-backed `user_id` values for direct-channel resolve/create + immediate thread open, and static fake-id DM fallback rows were replaced with roster-derived recipients. Team Chat hardbind/routing and role-policy gates preserved. Validation pass: `cd backend && npm run -s build`; `cd backend && npm run -s test:m6-comms-recipient-scope-hardening` (includes UUID payload snippet + team roster contract check); `cd app && npx tsc --noEmit --pretty false`. Evidence folder: `app/test-results/m6-dm-real-team-members-unblock-a/`. |
...
| `M6-CHALLENGE-DETAIL-DATA-CONTRACT-INVENTORY-B` | `committed` | `M6 challenge detail UI cleanup` | `Team Leader`, `Team Member`, `Controller` | `challenge detail data readiness` (`existing payload field inventory + fallback map`) | docs-only: `docs/spec/04_api_contracts.md`, `docs/spec/05_acceptance_tests.md`, `architecture/AGENT_ASSIGNMENT_BOARD.md` assignment note | `Coach-1` | `codex/a2-admin-list-usability-pass` (docs-only) | N/A (contracts/acceptance docs) | Accepted docs pass: challenge-detail support/fallback matrix landed for team goal, individual goal, leaderboard, and KPI contribution drill-in using existing challenge endpoint families only (no backend/schema changes) (`77fe5a3`). |
...
| `M6-CHALLENGE-FIRST-BUILDOUT-A` | `review` | `M6 challenge-first + pricing/entitlements direct migration` | `Solo User`, `Team Member`, `Team Leader`, `Coach`, `Sponsor` | `challenge-first nav + monetization guardrails` | `KPIDashboardScreen`, entitlement/paywall context/components, backend tier/billing/geo/custom-kpi/challenge policy routes | `Controller` | `codex/a2-admin-list-usability-pass` | owner-approved M6 direct migration plan | In-flight controller implementation includes migrations `021-024`, `/me` entitlements, billing/webhook routes, sponsored geo filters, custom KPI APIs, and solo challenge-first tab routing; pending final owner review + commit/push. |
| `M6-AVATAR-DROPDOWN-PROFILE-GOALS-SETTINGS-INVITES-A` | `review` | `M6 account-surface routing + invite wiring` | `Team Member`, `Team Leader`, `Coach`, `Solo User`, `Sponsor`, `Admin operator` | `avatar menu + split account screens + invite redemption` | `HomeScreen`, `KPIDashboardScreen`, `ProfileScreen`, `GoalsScreen`, `SettingsScreen`, `InviteCodeScreen`; backend `/me`, avatar upload, invite-code issue/redeem routes | `Controller` | `codex/a2-admin-list-usability-pass` | owner-approved M6 plan (fully wired) | Implemented in one swath: universal top-right avatar menu routing across top-level tabs, split profile/goals/settings/invite screens, signed avatar upload endpoint, invite code schema + issue/redeem endpoints (`team|coach|challenge`) with idempotent join semantics, and paired spec/decision docs updates; pending final owner review + commit/push. |
...
- `Execution note (2026-02-27 follow-on lock):` Board updated first and assignment re-opened to implement hard behavior: Team Leader KPI member-detail tap routes to Log with explicit `member_id`, `kpi_id`, and `source=team_leader_member_detail`, with KPI progress context rendering in the Log top block.
- `Completion note (2026-02-27 follow-on lock):` Team Leader member-row KPI-detail interaction now routes directly to Log using team-context handoff (`member_id`, `kpi_id`, `source=team_leader_member_detail`) instead of staying in Team KPI detail.
- `Completion note (2026-02-27 follow-on lock):` Log top context block now renders member name + KPI name + period and applies goal lookup priority (`onboarding goal` -> `profile goal` -> fallback summary copy) with progress-meter display when a goal target exists.
- `Completion note (2026-02-27 follow-on lock):` Team-context badge/icon behavior is preserved in Log quick-log tiles for handoff-selected KPIs.
- `Validation note (2026-02-27 follow-on lock):` `cd /Users/jon/compass-kpi/app && npx tsc --noEmit --pretty false` passed.
- `Validation note (2026-02-27 follow-on lock):` Screenshot evidence captured under `/Users/jon/compass-kpi/app/test-results/w12-team-kpi-member-log-routing/` (`team-kpi-log-1.png`, `team-kpi-log-2.png`, `team-kpi-log-3.png`).
- `Execution note (2026-02-27):` Assignment created and set to `active` before coding.
...
### `COACHING-W12-PORTAL-FOLDER-IA-DOCS-SYNC-D`

#### Snapshot
- `Status:` `committed`
- `Program status:` `M6/W12 coach portal folder/file IA docs sync (post Admin-1 correction)`
- `Persona:` `Coach` (primary), `Team Leader` (team-scoped), `Challenge Sponsor` (sponsor-scoped), `Admin operator` (governance)
- `Flow:` `coach portal authoring IA docs sync` (`Collections -> Assets`, `no duplicate assets tabs`, `2-column Journeys`, `mandatory drag/drop runtime`)
- `Owner:` `Coach-1`
- `Branch/worktree:` `codex/a2-admin-list-usability-pass` (docs-only; separate worktree preferred)
- `Execution note (2026-02-27):` Board updated first and assignment activated before doc edits.
- `Current blocker status (2026-02-27 start):` `none`.
- `Completion note (2026-02-27):` Updated `/Users/jon/compass-kpi/docs/spec/appendix/INTENDED_PERSONA_FLOW_SCREENMAP.md`, `/Users/jon/compass-kpi/docs/spec/appendix/INTENDED_WIRING_DIAGRAM.md`, and `/Users/jon/compass-kpi/docs/spec/appendix/COACHING_W12_DRAGDROP_LIBRARY_TO_JOURNEY_SPEC.md` to reflect corrected IA: Library folder/files model (`Collections -> Assets`), removal of duplicate Assets tab framing, Journey Builder two-column authoring layout (`source pane + journey builder`), and mandatory drag/drop runtime behaviors (`asset<->collection`, `asset->journey`, `reorder/remove`).
- `Validation note (2026-02-27):` Cross-doc terminology and behavior consistency verified across screenmap/wiring/dragdrop spec for folder/files IA, two-column builder layout, and mandatory drag/drop behavior set.
- `Decision note (2026-02-27):` `not needed` — no structural boundary change in this docs-sync pass.
- `Current blocker status (2026-02-27 completion):` `none`.

#### Primary Objective
Sync docs to corrected coach-portal authoring IA: Library as folder/files (`Collections -> Assets`) and Journey Builder as a two-column authoring layout with mandatory runtime drag/drop behaviors.

#### Hard Constraints
- Docs-only scope.
- No net-new endpoint family or schema changes.
- Preserve role boundaries and sponsor no-KPI-logging rule.
- Keep `/admin/coaching/audit` secondary-only governance/troubleshooting.
- Update `DECISIONS_LOG.md` only if this pass introduces structural boundary changes.

#### Required Work
1. Update `/Users/jon/compass-kpi/docs/spec/appendix/INTENDED_PERSONA_FLOW_SCREENMAP.md`:
   - Library = folder/files model (`Collections -> Assets`).
   - No duplicate Assets tab model.
   - Journeys = two-column authoring layout (`source pane + journey builder`).
   - Mandatory drag/drop runtime behaviors (`asset<->collection`, `asset->journey`, `reorder/remove`).
2. Update `/Users/jon/compass-kpi/docs/spec/appendix/INTENDED_WIRING_DIAGRAM.md` with matching IA/layout/interaction language.
3. Update `/Users/jon/compass-kpi/docs/spec/appendix/COACHING_W12_DRAGDROP_LIBRARY_TO_JOURNEY_SPEC.md` to align UX baseline/flow/build notes with corrected folder/files + two-column model.
4. Update `DECISIONS_LOG.md` only if structural boundary changes are introduced (expected: not needed for docs sync).

#### Required Validation
- Cross-doc wording consistency for:
  - `Collections -> Assets` folder/files model
  - no duplicate assets tab framing
  - two-column Journey authoring layout
  - mandatory drag/drop behavior set (`asset<->collection`, `asset->journey`, `reorder/remove`)
- Confirm no backend/API/schema changes introduced.

#### Report-Back Requirements
- Update assignment status/blocker/completion notes first (`committed`/`blocked`)
- Exact docs changed
- `DECISIONS_LOG` update status (`needed`/`not needed`)
- Commit hash

### `COACHING-W12-COACH-PORTAL-WORKSPACE-IA-DOCS-E`

#### Snapshot
- `Status:` `committed`
- `Program status:` `M6/W12 coach portal workspace IA canonicalization (docs/control-plane)`
- `Persona:` `Coach` (primary), `Team Leader` (team-scoped), `Challenge Sponsor` (sponsor-scoped), `Admin operator` (governance)
- `Flow:` `coach portal workspace IA docs sync` (`Journeys + People top-level modes`, `folder/files rules`, `asset->lesson/task drag/drop mapping`)
- `Owner:` `Coach-1`
- `Branch/worktree:` `codex/a2-admin-list-usability-pass` (docs-only; separate worktree preferred)
- `Execution note (2026-02-27):` Board updated first and assignment activated before doc edits.
- `Current blocker status (2026-02-27 start):` `none`.
- `Completion note (2026-02-27):` Updated `/Users/jon/compass-kpi/docs/spec/appendix/INTENDED_PERSONA_FLOW_SCREENMAP.md`, `/Users/jon/compass-kpi/docs/spec/appendix/INTENDED_WIRING_DIAGRAM.md`, and `/Users/jon/compass-kpi/docs/spec/appendix/COACHING_W12_DRAGDROP_LIBRARY_TO_JOURNEY_SPEC.md` to reflect canonical two-mode coach workspace IA: top-level `Journeys` and `People` only; Journeys split workspace (`Library tree/source + Builder`); People split workspace (`People list + Cohorts/Channels`); folder/files rules (`Collections` with nested assets, multi-expand); and mandatory asset->lesson/task drag/drop mapping.
- `Validation note (2026-02-27):` Cross-doc consistency check passed for workspace-mode terminology, split-layout definitions, folder/files rules, and drag/drop mapping semantics.
- `Decision note (2026-02-27):` `needed` — structural IA boundary classification changed; logged in `DEC-0049` at `/Users/jon/compass-kpi/architecture/DECISIONS_LOG.md`.
- `Current blocker status (2026-02-27 completion):` `none`.

#### Primary Objective
Sync docs to the canonical coach portal workspace IA with only two top-level modes (`Journeys`, `People`) and explicit behavior rules for folder/files navigation and drag/drop mapping into lessons/tasks.
...
- Preserve role boundaries and sponsor no-KPI-logging rule.
- Keep `/admin/coaching/audit` secondary-only governance/troubleshooting.
- Update `DECISIONS_LOG.md` if workspace IA reclassification changes structural boundary classification.

#### Required Work
1. Update `/Users/jon/compass-kpi/docs/spec/appendix/INTENDED_PERSONA_FLOW_SCREENMAP.md`:
   - two top-level coach modes only: `Journeys`, `People`
   - Journeys workspace = Library tree + Journey builder split
   - People workspace = People list + Cohorts/Channels split
   - folder/file behavior rules (collections with nested assets; multi-expand)
   - drag/drop mapping from assets to lessons/tasks
2. Update `/Users/jon/compass-kpi/docs/spec/appendix/INTENDED_WIRING_DIAGRAM.md` with matching IA and route/workspace mapping language.
3. Update `/Users/jon/compass-kpi/docs/spec/appendix/COACHING_W12_DRAGDROP_LIBRARY_TO_JOURNEY_SPEC.md` with workspace-model terminology and drag/drop mapping updates.
4. Update `/Users/jon/compass-kpi/architecture/DECISIONS_LOG.md` only if boundary classification changed.

#### Required Validation
- Cross-doc consistency for:
  - two-mode workspace IA (`Journeys`, `People`)
  - Journey workspace split (Library tree + Builder)
  - People workspace split (People list + Cohorts/Channels)
  - folder/files behavior (nested assets, multi-expand)
  - drag/drop mapping from assets to lessons/tasks
- Confirm no backend/API/schema changes introduced.
...
- `Owner:` `Mobile-1`
- `Branch/worktree:` `codex/a2-admin-list-usability-pass` (dedicated mobile worktree required)
- `Figma refs:` `manual-spec-driven + Fourth Reason comms IA parity direction`
- `Worker note (2026-02-27, Mobile-1, execution):` Implemented W12 Comms Hub IA alignment in `KPIDashboardScreen.tsx`: primary tabs (`All`, `Channels`, `DMs`, `Broadcast`), role-gated broadcast tab and destination targeting, channels scope filters, tab/filter-aware search placeholder behavior, channel scope/type/activity chips, and DM direct-thread open rows while preserving route contracts and endpoint families.
- `Current blocker status (2026-02-27, Mobile-1, execution):` `none` for implementation/compile scope.

#### Primary Objective
Align mobile Comms Hub runtime IA to Fourth Reason-style structure while preserving existing runtime routes, endpoint-family boundaries, and sponsor/role policy guardrails.

#### Hard Constraints
- No backend/API/schema changes.
- Keep existing endpoint families and routes intact.
- Preserve sponsor no-KPI-logging rule.
- Preserve Home/Priority KPI logging mechanics.
...
- board status first
- files + line refs
- tab/filter/permission map
- commit hash
- `Completion note (2026-02-27, Mobile-1):` Completed Comms Hub IA alignment pass: tabbed IA + scope filters + role-gated broadcast target controls landed in `KPIDashboardScreen.tsx` with no backend/schema/API changes.
- `Guardrail note (2026-02-27, Mobile-1):` Existing endpoint families (`/api/channels*`, `/api/messages*`, `/api/coaching*`, `/api/ai/suggestions*`) unchanged; sponsor no-KPI-logging boundary preserved.
- `Validation note (2026-02-27, Mobile-1):` `cd /Users/jon/compass-kpi/app && npx tsc --noEmit --pretty false` passed.
- `Screenshot note (2026-02-27, Mobile-1):` captured runtime screenshot:
  - `/Users/jon/compass-kpi/app/test-results/w12-comms-hub-ia-fourthreason-align-a/comms-current.png`
- `Screenshot blocker note (2026-02-27, Mobile-1):` scripted Simulator state navigation (for All/Channels/DMs/Broadcast and persona permutations) remains blocked in this environment by macOS assistive-access denial for `osascript` click automation (`-25211`); direct `simctl` screenshot capture is functional.
...
- `Owner:` `Mobile-1`
- `Branch/worktree:` `codex/a2-admin-list-usability-pass` (dedicated mobile worktree required)
- `Figma refs:` `manual-spec-driven + accepted W12 comms IA alignment baseline`
- `Worker note (2026-02-27, Mobile-1, start):` Board updated first. Starting cleanup pass to keep Comms messaging-only, remove journey controls from Channels, remove duplicate thread entry controls, and suppress empty channel notification rows.
- `Current blocker status (2026-02-27, Mobile-1, start):` `none`.

#### Primary Objective
Reduce Comms hub clutter so Channels has one thread-entry pattern and messaging-only controls while preserving journeys as separate destination surfaces.

#### Hard Constraints
- No backend/API/schema changes.
- Preserve existing route families.
- Preserve role gates and sponsor no-KPI-logging rule.
...
#### Report-Back
- files + line refs
- controls removed/kept map
- commit hash
- `Completion note (2026-02-27, Mobile-1):` Completed cleanup pass in `KPIDashboardScreen.tsx`: Comms remains messaging-only, removed `Journeys` and `Open Thread/Open Channel Thread` control clutter from Channels-oriented controls, retained row tap/chevron as the single thread-entry pattern, and hid channel notification rows section when empty.
- `Guardrail note (2026-02-27, Mobile-1):` No backend/schema/API changes; existing route families and sponsor no-KPI-logging + role gates preserved.
- `Validation note (2026-02-27, Mobile-1):` `cd /Users/jon/compass-kpi/app && npx tsc --noEmit --pretty false` currently fails on unrelated pre-existing web file error: `screens/CoachPortalScreen.tsx` missing `COACH_SURFACES` symbol (`TS2304`).
- `Screenshot note (2026-02-27, Mobile-1):` captured current runtime evidence:
  - `/Users/jon/compass-kpi/app/test-results/w12-comms-hub-cleanup-legacy-controls-f/comms-cleanup-current.png`
- `Screenshot blocker note (2026-02-27, Mobile-1):` scripted Simulator state switching for full `All/Channels/DMs/Broadcast` and role-variant screenshot matrix remains blocked by macOS assistive-access denial for `osascript` (`-25211`) in this environment.
...
- `Completion note (2026-02-27, Admin-1):` Implemented owner correction package in `CoachPortalScreen`: Library now renders canonical `Assets` + `Collections` workspace, journey builder exposes `Create New Journey` (blank draft + name), drag/drop interactions support asset->collection and asset/collection-item->milestone assignment in web runtime, reorder/remove controls remain functional, and Save Draft moved to builder action bar with explicit `IDLE/PENDING/SAVED/ERROR` feedback.
- `Validation note (2026-02-27, Admin-1):` `cd /Users/jon/compass-kpi/app && npm run test:unit -- adminAuthz` passed; `cd /Users/jon/compass-kpi/app && npx tsc --noEmit --pretty false` passed; runtime checks executed against `http://localhost:19009` for `/coach/library`, `/coach/journeys`, `/coach/uploads` redirect, and `/admin/coaching/library` compatibility redirect with screenshot evidence.
...
### `COACHING-W12-PORTAL-INTERACTION-PRIMITIVES-C`

#### Snapshot
- `Status:` `committed`
- `Program status:` `M6 W12 coach portal interaction primitive correction pass`
- `Persona:` `Coach` (primary), `Team Leader` (team-scoped authoring), `Challenge Sponsor` (sponsor-scoped read-only)
- `Flow:` `coach portal interaction UX` (`scroll reliability`, `tabs/rows/menus over button-only IA`, `journey builder primitive clarity`)
- `Owner:` `Admin-1`
- `Branch/worktree:` `codex/a2-admin-list-usability-pass` (coach/admin web worktree preferred)
- `Worker note (2026-02-27, Admin-1, start):` Board updated first. Starting interaction-primitives correction pass for `/coach/library` and `/coach/journeys` to enforce scroll reliability and intent-specific controls (tabs/segments, row selection, menus), while preserving drag/drop + role gates.
- `Current blocker status (2026-02-27, Admin-1, start):` `none`.
- `Completion note (2026-02-27, Admin-1):` Implemented interaction-primitives correction in `CoachPortalScreen`: added reliable page scroll container behavior for coach portal runtime, replaced button-heavy section/source switching with segmented tabs, preserved row-click selection patterns for assets/journeys, replaced per-step secondary action buttons with context menu (`Actions ▾`), and retained primary commit buttons (`Create New Journey`, `Save Draft`) in builder action bar.
- `Validation note (2026-02-27, Admin-1):` `cd /Users/jon/compass-kpi/app && npm run test:unit -- adminAuthz` passed; `cd /Users/jon/compass-kpi/app && npx tsc --noEmit --pretty false` passed; runtime check script confirmed scroll movement, keyboard tab focus progression, primitive clarity, drag/drop + menu behavior, and compatibility redirects on `/coach/library` + `/coach/journeys`.
- `Current blocker status (2026-02-27, Admin-1, completion):` `none`.

#### Primary Objective
Remove button-only interaction patterns and enforce intent-aligned interaction primitives with fully scrollable coach portal views.

#### Hard Constraints
- No backend/schema/API changes.
- Keep role gates intact and sponsor no-KPI-logging rule unchanged.
- Keep `/coach/uploads` and `/admin/coaching/*` compatibility redirects intact.

#### Required Work
1. Ensure desktop/laptop scroll reliability with no clipped long panels.
2. Replace button-heavy IA with:
   - tabs/segmented controls for section switching
   - row click/select for list selection
   - dropdown/menu for secondary actions
   - buttons reserved for primary commits
3. Journey builder:
   - keep `Create New Journey` visible and primary
   - keep `Save Draft` in builder action bar
   - preserve drag/drop and add non-button affordances (row select + context menu)
...
- Runtime checks:
  ...
  - primary/secondary action clarity
- Screenshots:
  - `/coach/library`
  - `/coach/journeys`
  - show tabs/menus/list selection in evidence
...
### `COACHING-W12-PORTAL-FOLDER-IA-CORRECTION-D`

#### Snapshot
- `Status:` `committed`
- `Program status:` `M6 W12 coach portal folder/files IA correction pass`
- `Persona:` `Coach` (primary), `Team Leader` (team-scoped authoring), `Challenge Sponsor` (sponsor-scoped read-only)
- `Flow:` `coach portal authoring UX` (`true folder/files manager behavior`, `2-column journeys source->builder`, `runtime drag/drop reliability`)
- `Owner:` `Admin-1`
- `Branch/worktree:` `codex/a2-admin-list-usability-pass` (coach/admin web worktree preferred)
- `Worker note (2026-02-27, Admin-1, start):` Board updated first. Starting D correction pass to remove duplicate source panes/tabs and enforce canonical folder/files behavior in Library + 2-column Journey Builder IA.
- `Current blocker status (2026-02-27, Admin-1, start):` `none`.
- `Completion note (2026-02-27, Admin-1):` Applied owner IA corrections in `CoachPortalScreen`: Library now behaves as a normal folder/files manager (left folders list only + main files pane for selected folder), Journeys now renders as strict 2-column source/builder layout, redundant source columns/tabs were removed, and `Create New Journey` + `Save Draft` remain in builder action bar with explicit `idle/pending/saved/error` status.
- `Validation note (2026-02-27, Admin-1):` `cd /Users/jon/compass-kpi/app && npm run test:unit -- adminAuthz` passed; `cd /Users/jon/compass-kpi/app && npx tsc --noEmit --pretty false` passed; runtime checks on `/coach/library` and `/coach/journeys` passed for asset->collection, asset->milestone, and reorder/remove menu states with screenshots under `app/test-results/w12-portal-folder-ia-correction-d/`.
- `Current blocker status (2026-02-27, Admin-1, completion):` `none`.

#### Primary Objective
Apply owner-required folder/files IA correction so `/coach/library` behaves as a normal folder manager and `/coach/journeys` is a strict 2-column source/builder authoring surface.

#### Hard Constraints
- No backend/schema/API changes unless blocked.
- Preserve role gates and sponsor no-KPI-logging boundaries.
- Preserve `/coach/uploads` and `/admin/coaching/*` compatibility redirects.

#### Required Work
1. Library:
   - left pane is collections tree/list only
   - main pane shows files/assets for selected collection
   - remove duplicated assets-tab style rendering and extra columns
   - keep runtime drag/drop asset -> collection functional
2. Journeys:
   - enforce exactly 2 columns:
     - column A: single navigable Library source pane (collections + selected collection assets)
     - column B: Journey Builder (journey list + milestones/steps)
   - remove extra source columns/tabs
3. Interaction simplification:
   - remove button-heavy section switching where row/tree interaction should drive navigation
   - keep `Create New Journey` and `Save Draft` in builder action bar
   - keep save state explicit (`idle/pending/saved/error`)
4. Drag/drop reliability:
   - asset -> collection
   - asset -> journey milestone
   - reorder/remove in milestone
...
- Runtime checks:
  ...
  - `/coach/journeys`
- Screenshot proof:
  - folder/files library view
  - 2-column journeys builder
  - drag/drop behavior evidence

#### Report-Back
- update board status first
- files + line refs
- before/after IA map
- drag/drop behavior checklist (`pass/fail`)
- commit hash
...
### `ADMIN-W12-INTERACTION-PRIMITIVES-DOCS-SYNC-C`

#### Snapshot
- `Status:` `committed`
- `Program status:` `A3/W12 admin interaction primitives docs sync`
- `Persona:` `Admin operator` (primary), `Coach` (admin-hosted companion routes)
- `Flow:` `admin interaction model` (`commit buttons`, `view switching controls`, `row-selection/sorting`, `secondary-action menus`, `scroll requirements`)
- `Owner:` `Coach-1`
- `Branch/worktree:` `codex/a2-admin-list-usability-pass` (docs-only; separate worktree preferred)
- `Dependency note:` Execute after Admin-1 lands. This pass is docs-only and standardizes interaction primitives for admin-hosted surfaces.
- `Worker note (2026-02-27, Coach-1, start):` Board updated first. Starting admin interaction-primitives docs sync for screenmap/wiring/board coverage.
- `Current blocker status (2026-02-27, Coach-1, start):` `none` for docs-only scope.
- `Completion note (2026-02-27, Coach-1):` Synced screenmap/wiring docs to admin interaction-primitives standard: primary buttons reserved for commit actions, tabs/segmented controls for view switching, row-selection + table-header sorting patterns defined, menus reserved for secondary actions, and route-level scroll behavior requirements defined.
- `Validation note (2026-02-27, Coach-1):` Cross-doc wording consistency verified between board assignment notes and the new admin primitives sections in screenmap/wiring.
- `Decision log note (2026-02-27, Coach-1):` `DECISIONS_LOG.md` not updated; no structural boundary changed.
- `Current blocker status (2026-02-27, Coach-1, completion):` `none`.

#### Primary Objective
Lock a shared admin interaction-primitives standard so admin implementation lanes follow the same control behavior conventions across admin web surfaces.

#### Hard Constraints
- No backend/schema/API endpoint-family changes.
- Preserve current role boundaries and route ownership.
- Keep `/admin/coaching/audit` secondary-only governance/troubleshooting.
- Update `DECISIONS_LOG.md` only if structural boundaries change.

#### Required Work
1. Update screenmap/wiring docs with admin interaction-primitives standard:
   - primary buttons only for commit actions
   - tabs/segmented controls for view switching
   - row-selection/table-header sorting patterns
   - menus for secondary actions
   - scroll behavior requirement
2. Update board assignment row/block status and notes after docs sync.

#### Validation
- Cross-doc consistency check between board/screenmap/wiring for shared admin primitive language.
- Confirm no structural boundary delta requiring `DECISIONS_LOG.md`.
...
#### Scope In
- Challenge landing state model and top switcher (`Active`, `Upcoming`, `History`)
- Active challenge default selection and quick challenge switching behavior
- Team-pattern active view structure (participant rows, KPI summary, role-aware actions)
- Clean navigation paths into challenge details and leaderboard

#### Scope Out
- Team screen rewrite
- Home/Priority rewrites
- backend/API/schema changes

#### Hard Constraints
- Keep existing challenge join/leave endpoint usage intact.
- Do not modify Team surface except shared local helper/style reuse if required.
- No structural/backend boundary changes.

#### Validation
- `cd /Users/jon/compass-kpi/app && npx tsc --noEmit --pretty false`
- Runtime checks:
  - opens to active challenge by default
  - switches among Active/Upcoming/History correctly
  - details/leaderboard paths work from relevant states
...
- `Execution note (2026-02-28, Mobile-2):` Board checked first; assignment executed as QA-only (no code edits) with deterministic matrix assembly and consolidated screenshot evidence capture.
- `Completion note (2026-02-28, Mobile-2):` Built persona x route regression matrix for `Home`, `Team`, `Challenge`, `Comms`, `Journeys/Lessons`, `Log` across `Team Leader`, `Team Member`, `Solo User`, `Coach`, `Challenge Sponsor` and finalized matrix output at `/Users/jon/compass-kpi/app/test-results/m8-mobile-regression-matrix-a/runtime-matrix.md`.
- `Completion note (2026-02-28, Mobile-2):` Consolidated route evidence screenshots under `/Users/jon/compass-kpi/app/test-results/m8-mobile-regression-matrix-a/` (`home-baseline.png`, `team-leader-view.png`, `team-member-view.png`, `challenge-list-member.png`, `challenge-detail-member.png`, `challenge-leaderboard-member.png`, `comms-hub.png`, `journeys-lessons.png`, `log-team-context.png`).
- `Validation note (2026-02-28, Mobile-2):` `cd /Users/jon/compass-kpi/app && npx tsc --noEmit --pretty false` passed.
- `Validation note (2026-02-28, Mobile-2):` Matrix summary: `30 pass`, `0 fail`, `0 blockers`.
...
- `Relaunch note (2026-02-28, Mobile-2):` Assignment re-opened for exact re-execution per controller directive; board set to `active` before command run.
- `Completion note (2026-02-28, Mobile-2 re-execution):` Re-ran deterministic commands from `/backend` exactly per runbook: `npm run seed:coaching:realism:reset`, `npm run seed:coaching:realism`, and `npm run seed:coaching:realism:smoke`. All commands passed and returned expected deterministic dataset coverage and persona visibility checks.
- `Validation note (2026-02-28, Mobile-2 re-execution):` No command/output drift observed relative to `/docs/spec/appendix/M6_REALISM_DATA_PACK_UI_EVALUATION.md`; refreshed logs captured at `/Users/jon/compass-kpi/app/test-results/m8-seed-data-smoke-verification-a/04-seed-reset-rerun.log`, `/Users/jon/compass-kpi/app/test-results/m8-seed-data-smoke-verification-a/05-seed-rerun.log`, and `/Users/jon/compass-kpi/app/test-results/m8-seed-data-smoke-verification-a/06-smoke-only-rerun.log`.
- `Current blocker status (2026-02-27, launch):` `none`.
...
- `Completion note (2026-02-28, Worker+Controller):` Implementation complete and accepted. Team identity card is now polished and role-aware: leader-only edit control opens a modal sheet with avatar/background options (with preview, cancel, save); member sees read-only configured result; team chat appears on both leader/member identity card; Invite Member remains leader-only. TypeScript clean (`npx tsc --noEmit --pretty false` = 0 errors).
- `Current blocker status (2026-02-28, complete):` `none`.
...
- `cd /Users/jon/compass-kpi/app && npx tsc --noEmit --pretty false`
- Runtime checks:
  - Team Leader sees edit control, can open/close editor, save/cancel changes.
...
- `Execution note (2026-02-28, Controller):` Owner approved expanded scope fold-in after identity-card pass; merge completed in same safe handoff window with no ownership collision.
- `Completion note (2026-02-28, Worker+Controller):` Team Focus KPI + Team Focus projections are now combined into one section for leader/member, legacy focus pill-only block removed, and leader uses compact explicit edit control to manage selected focus KPIs.
- `Current blocker status (2026-02-28, complete):` `none`.
...
- `Execution note (2026-02-28, Controller follow-on):` Owner requested removal of `KPI Concern Flags` card in Team Leader view because concern handling now exists in other surfaces.
...
- `Completion note (2026-02-28, Mobile-1 follow-on):` Removed the redundant Team Leader `KPI Concern Flags` card while keeping Team Health Summary placement and member list interactions intact.
...
- `Validation note (2026-02-28, Mobile-1 follow-on):` Updated screenshot evidence captured at `/Users/jon/compass-kpi/app/test-results/m6-team-leader-health-summary-restore-a/team-leader-after-no-concern-card.png`.
- `Commit refs:` `a45c0bd`, `2f5f1d6`
- `Current blocker status (2026-02-28, launch):` `none`.
- `Current blocker status (2026-02-28, completion):` `none`.

#### Primary Objective
Keep the Team Leader team-level health summary card/module directly above the Team Members list and remove the redundant `KPI Concern Flags` card from Team Leader view.

#### Scope In
- Team Leader Team tab only.
- Maintain team-level health summary module placement above Team Members list.
- Remove Team Leader `KPI Concern Flags` card.
...
- Runtime checks on Team Leader persona:
  ...
  - `KPI Concern Flags` card is absent from Team Leader view
...
- `Completion note (2026-02-28, Coach-1):` Updated control-plane dependency register and W13 closeout tracker so `DEP-002`, `DEP-004`, and `DEP-005` are all `CLOSED`; added explicit owner risk-acceptance note (`2026-02-28`) covering residual tenancy/compliance/vendor operational risk.
- `Validation note (2026-02-28, Coach-1):` Wave A GO/NO-GO matrix now consistent with closed dependency/evidence/sign-off statuses (`CLOSED` + `PRESENT` + `SIGNED` => overall `GO`).
- `Current blocker status (2026-02-28, committed):` `none` (docs-only scope).
...
- `Completion note (2026-02-28, Controller checkpoint sync):` Runtime checkpoint returns accepted into control-plane review with landed runtime commits `c8d85ea`, `dd89e15`, `103d725`, and `8cdfa78`; board status advanced from `queued` to `review` pending final acceptance pass.
- `Execution note (2026-02-28, Mobile-2 Checkpoint C start):` Starting Mux Wave B implementation pass for upload session flow, playback token flow, webhook verification/lifecycle handling, and parity/failure-path validation coverage.
- `Completion note (2026-02-28, Mobile-2 Checkpoint C):` Implemented runtime endpoints `POST /api/coaching/media/upload-url`, `POST /api/coaching/media/playback-token`, and `POST /api/webhooks/mux` with deterministic error envelopes, role gating, replay/signature checks, duplicate webhook idempotency, lifecycle transition guards, and no KPI side-effect mutation paths.
...
- `Completion note (2026-02-28, Mobile-1):` Replaced Team Challenges static row shell with a data-driven hero + compact card layout, added active/completed segment filtering, and wired non-joined challenge rows to a bottom-drawer preview while preserving joined-row direct route to existing challenge details.
- `Validation note (2026-02-28, Mobile-1):` `cd /Users/jon/compass-kpi/app && npx tsc --noEmit --pretty false` passed.
- `Validation note (2026-02-28, Mobile-1):` Runtime artifacts captured at `/Users/jon/compass-kpi/app/test-results/m6-team-challenges-list-hero-drawer-a/` (`before-team-challenges-list.png`, `after-team-challenges-runtime.png`).
- `Current blocker status (2026-02-28, completion):` `none`.
...
### `M6-CHALLENGE-DETAIL-DATA-CONTRACT-INVENTORY-B`

#### Snapshot
- `Status:` `review`
- `Program status:` `M6 challenge detail UI cleanup`
- `Persona:` `Team Leader`, `Team Member`, `Controller`
- `Flow:` `challenge detail data readiness` (`existing payload field inventory + fallback map`)
- `Owner:` `Coach-1`
- `Current blocker status (2026-02-28, launch):` `none` (docs-only).
- `Completion note (2026-02-28, Coach-1):` Added docs-first contract inventory in `04_api_contracts.md` for current challenge payload fields (`my_participation`, `leaderboard_top`, join response participant fields) and explicit fallback rendering rules for unsupported team aggregate + KPI contribution drill-in states.
- `Validation note (2026-02-28, Coach-1):` Added acceptance scenario `#7A` and updated M4 harness mapping to include challenge-detail fallback safety; all references remain inside existing challenge endpoint families.
- `Evidence note (2026-02-28, Coach-1):` Canonical inventory sections are `docs/spec/04_api_contracts.md` ("M6 challenge detail MVP data-contract inventory") and `docs/spec/05_acceptance_tests.md` (`7A` + updated M4 mapping + regression checklist fallback line).
- `Current blocker status (2026-02-28, review):` `none` (docs-only).

#### Primary Objective
Support the Challenge Detail MVP redesign with a docs-first inventory of existing data so Claude can ship UI without backend drift:
1. Enumerate existing challenge payload fields currently available to challenge detail runtime for:
   - team-goal progress
   - individual-goal progress
   - leaderboard ranking
   - KPI-level contribution drill-in
2. Mark which requested UI states are fully supported vs partially supported.
3. Define explicit fallback rendering rules for unsupported fields.

#### Scope In
- `/Users/jon/compass-kpi/docs/spec/04_api_contracts.md`
- `/Users/jon/compass-kpi/docs/spec/05_acceptance_tests.md`
- `/Users/jon/compass-kpi/architecture/AGENT_ASSIGNMENT_BOARD.md` (assignment evidence note only)

#### Scope Out
- Backend endpoint changes.
- Schema/table changes.
- Runtime UI code edits.

#### Hard Constraints
- No new endpoint families.
- No structural changes requiring decision-log updates.
- Keep outputs implementation-directed and concise.

#### Required Validation
- Contract references must point to existing endpoint families already in `04_api_contracts.md`.
- Acceptance scenarios added/updated must remain in-sprint and UI-fallback-safe.

#### Report-Back
- Updated files + exact sections.
- Supported vs fallback data matrix summary.
- Commit hash.
...
- `Owner:` `Coach-1`
- `Completion note (2026-02-28, Admin-1):` Replaced placeholder DEP-004 evidence reference in control-plane tracker with concrete artifact path, added explicit cross-link between control-plane tracker and W13 sign-off brief, and kept Wave A GO/NO-GO gate language explicit and unchanged.
- `Current blocker status (2026-02-28, completion):` `none` (docs-only).
...
- `/Users/jon/compass-kpi/architecture/W13_DEP_SIGNOFF_BRIEF.md`
...
- Confirm no status contradiction between control-plane tracker and W13 signoff brief.
...
1. Leaderboard hero card at top.
2. `Goals` section with KPI rows styled like profile cards:
   - KPI identity on left
   - KPI progress/meta on right
3. Two explicit goal groups:
   - `Team Goals`
   - `Individual Goals`
4. Team goal progress meters represent cumulative team contribution.
5. Individual goal progress meters represent only current user progress.
6. Tapping any KPI row opens KPI contribution breakdown (participant-level contributions for that KPI).

#### Scope In
- `/Users/jon/compass-kpi/app/screens/KPIDashboardScreen.tsx` challenge detail composition only.
- Existing challenge detail/read-model data usage and safe fallback rendering.
- Visual hierarchy cleanup: remove non-essential buttons/noise from current screenshoted state.

#### Scope Out
- New tables, migrations, schema changes, or backend endpoint families.
- Stream Chat/provider runtime integration work.
- Challenge list page redesign (already covered by prior assignment).

#### Hard Constraints
- Use existing payloads/contracts only; if data is thin, use explicit MVP fallback copy/states.
- Do not create or require new database tables.
- Preserve existing join/leave and challenge route continuity.
- Keep role boundaries intact.
- If backend field is missing, render safe placeholder without inventing persistence paths.

#### Required Validation
- `cd /Users/jon/compass-kpi/app && npx tsc --noEmit --pretty false`
- Runtime checks:
  - leaderboard hero renders first
  - team goals and individual goals are visually distinct
  - team goal meter reflects aggregate team contribution
  - individual goal meter reflects current user only
  - KPI tap opens contribution breakdown view/sheet
- Screenshot evidence for leader + member personas.
...
### `M6-COMMS-FR-PATTERN-MATRIX-B`

#### Snapshot
- `Status:` `review`
- `Program status:` `M6 comms integration planning (docs-only)`
- `Persona:` `Coach`, `Team Leader`, `Team Member`, `Solo User`, `Challenge Sponsor`
- `Flow:` `comms architecture mapping` (`Fourth Reason behavior -> Compass adaptation matrix`)
- `Owner:` `Mobile-2`
- `Current blocker status (2026-02-28, launch):` `none` (docs-only).
- `Execution note (2026-02-28, Mobile-2):` Assignment activated from owner directive. Executing docs-only pattern matrix pass across Fourth Reason integration doc + additive API mapping notes.
- `Completion note (2026-02-28, Mobile-2):` Added a pattern-by-pattern messaging matrix mapping Fourth Reason behavior to Compass equivalents with explicit `build now / defer / blocked by DEP` outcomes and a mandatory no-new-table adaptation strategy per pattern.
- `Validation note (2026-02-28, Mobile-2):` Cross-checked statuses and dependency gates against `/Users/jon/compass-kpi/architecture/CURRENT_SPRINT.md` (W13 docs-first + dependency gates) and `/Users/jon/compass-kpi/architecture/NON_NEGOTIABLES.md` (deterministic, scope-safe constraints). No runtime/API family expansion introduced.
- `Current blocker status (2026-02-28, review):` `none`.

#### Primary Objective
Create a Compass implementation matrix from Fourth Reason messaging behavior:
- pattern-by-pattern mapping (`Fourth Reason behavior` -> `Compass equivalent`)
- explicit `build now / defer / blocked by DEP` status
- explicit `no new table` adaptation strategy for every pattern

#### Scope In
- `/Users/jon/compass-kpi/architecture/AGENT_ASSIGNMENT_BOARD.md`
- `/Users/jon/compass-kpi/docs/spec/appendix/FOURTH_REASON_INTEGRATION_MATRIX.md`
- `/Users/jon/compass-kpi/docs/spec/04_api_contracts.md` (additive mapping notes only)

#### Scope Out
- Runtime code changes
- Backend/API/schema implementation changes
- Net-new endpoint family design

#### Required Validation
- Cross-check against:
  - `/Users/jon/compass-kpi/architecture/CURRENT_SPRINT.md`
  - `/Users/jon/compass-kpi/architecture/NON_NEGOTIABLES.md`
- Confirm matrix explicitly marks dependency-gated items where blocked (`DEP-*`).
