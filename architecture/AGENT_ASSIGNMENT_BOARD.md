...
| `COACHING-W5-UI-AI-ASSIST-SHELL-PROTO-A` | `committed+pushed` | `M6 coaching slice (W5 AI assist UI shell/proto; approval-first)` | `Team Leader`, `Coach`, `Admin operator` (review context), downstream `Member/Solo` limited | `coaching / AI assist` (`runtime insert-point shells + review UX`) | `channel_thread`, `coach_broadcast_compose`, `coaching_lesson_detail`, `coaching_journeys*`, embedded Team/Challenge coaching CTAs (approved insert points only) | `Mobile-1` | `codex/a2-admin-list-usability-pass` (dedicated worktree preferred) | manual-spec-driven + W5 AI boundary docs | Accepted and pushed (`6916f88`): approval-first AI assist shell proto + review modal across approved mobile coaching insert points, with no autonomous send/publish and no KPI/forecast/challenge-state mutation paths. |
